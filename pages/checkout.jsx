...
const CheckoutPage = () => {
  ...
  const onSubmit = data => {
    ...
    itemss.map(it => {
      ...
      product_o.push({
        ...
        product_variant_name:
          it.variant_name == undefined ? '' : it.product_variant_id,
        product_name: it.name,
        price: it.price,
        path: x,
        quantity: it.count,
    ...
    if (data.payment == 'credit card') {
      ...
      axios
        .post(`http://localhost:8080/ecom-api/orders`, order_json)
        .then(res => {
  ...
  const getToken = () => {
    axios.get(`http://localhost:8080/ecom-api/payment`).then(res => {
  ...
  const handleApply = () => {
    axios
      .get(`http://localhost:8080/ecom-api/coupons/byuser`)
      .then(res => {
        let curr = new Date();
        setErrorDiv(true);
        let status = false;
        ...
        res.data.data.map(coupon => {
          if (coupon.coupon_code == coupon_code) {
            ...
            if (curr.getTime() > d.getTime()) {
              ...
              axios
                .get(
                  `http://localhost:8080/ecom-api/coupons/user_coupon_id/${ucid}`
                )
                .then(resp => {
                  console.log(resp);
                  if (resp.data.success == 0) {
                    ...
                    axios
                      .post(
                        `http://localhost:8080/ecom-api/coupons/user_coupon_id`,
                        { id: ucid, cid: coupon.id, usage: coupon.total_usage },
                      ...
                      .then(res => {
                        ...
                        if (coupon.discount_type == 'percentage') {
                          ...
                          let tt =
                            priceTotal - coupon.discount_value + shipprice;
                          console.log('helooo', priceTotal);
                          setPTotal(tt);
                    ...
                    if (
                      coupon.usage_limit_per_user == resp.data.data.user_limit
                    )
                      ...
                      axios
                        ...
                        .then(response => {
                          ...
                          if (coupon.discount_type == 'percentage') {
                            let dval =
                              (coupon.discount_value / 100) * priceTotal;
                            let tt = priceTotal - dval + shipprice;
                            setPTotal(tt);
                            setCVal(dval);
                          } else {
  ...
  const submitShipHandler = e => {
    ...
    axios
      .put(`http://localhost:8080/ecom-api/addresses/${editship.id}`, put_s)
      .then(response => {
        console.warn('rsponse', response);
      })
      .catch(err => console.log(err));
  ...
  const submitEditBillHandler = e => {
    ...
    // setTitle(false)
    axios
      .put(`http://localhost:8080/ecom-api/addresses/${address.id}`, billput)
      .then(response => {
        setBill(address);
        console.warn('rsponse', response);
      })
  ...
  const submitAddShipHandler = e => {
    ...
    if (addship.user_address == '') {
      ...
      axios
        .post(
          `http://localhost:8080/ecom-api/addresses/${user_Id}`,
          addship,
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('token')) {
      ...
      date = date.toString();
      let dd = '';
      for (let i = 0; i < 15; i++) dd = dd + date[i];
      setMyDate(dd);
      setBol(false);
      setAccount(true);
      ...
      axios
        .get(
          `http://localhost:8080/ecom-api/users/${u_id}`,
      ...
      axios
        .get(`http://localhost:8080/ecom-api/shipping`)
        .then(response => {
          console.log('shipping', response.data.data);
          response.data.data.map(sh => {
            if (sh.category == 'standard') setStandard(sh);
            else setPremium(sh);
          });
          console.log(user);
        })
  ...
  return (
    <Body>
      <TopLinks>
        <StyledLink href='/'>
          <Logo src='./pernia.png'></Logo>
        </StyledLink>
        <StyledLink href='/cart'>
          <TopBtn>Back to Cart</TopBtn>
        </StyledLink>
      </TopLinks>
      <div style={modal == true ? { opacity: '0' } : null}>
        ...
        {account == false ? (
          <StyledLink href='/account/login'>
            <Login>Already have an account? Click here to login</Login>
          </StyledLink>
        ) : (
          ''
        )}
        ...
        <>
          <Modal>
            <Detail>
              ...
              <p>
                Please keep the above numbers for your reference. We will also
                send the conformation to the email address you used for this
                order. Please allow up to 24 hours for us to process your order
                for shipment
              </p>
              <Address>
                <div>
            ...
            >
              <SummaryItemTitle
                style={{ marginLeft: '60px', fontWeight: '800' }}
              >
                Order Summary
              </SummaryItemTitle>
              <SummaryItem>
              ...
              <SummaryItem type='total'>
                ...
                <SummaryItemPrice>
                  <strong>{priceTotal.toFixed(2)} Rs</strong>
                </SummaryItemPrice>
              </SummaryItem>
              <SummaryItem>
                <GoButton color='primary' type='button' onClick={() => move()}>
                  Okay
                </GoButton>
              </SummaryItem>
            </OSummary>
      ...
      <Container
        style={modal == true || creditcard == true ? { opacity: '0' } : null}
      >
        <Form onSubmit={handleSubmit(onSubmit)}>
          <Left>
            {editdiv && (
              <FormDiv>
                <TitleBlock>
                  <HouseOutlined style={{ paddingRight: '50px' }} /> Shipping
                  Address
                </TitleBlock>
                <div>
                  <InputDiv>
                  ...
                  <InputDivC>
                    <InputDivS>
                      <Select
                        ...
                        onChange={handleChange}
                      >
                        <option value={editship.country} disabled selected>
                          {editship.country}
                        </option>
                    ...
                    <InputDivS>
                      <Select
                        ...
                        onChange={handleChange}
                      >
                        <option value={editship.province} disabled selected>
                          {editship.province}
                        </option>
                  ...
                  <InputDivC>
                    <InputDivS>
                      <Select
                        ...
                        onChange={handleChange}
                      >
                        <option value={editship.city} disabled selected>
                          {editship.city}{' '}
                        </option>
                        <option value='Islamabad'>Islamabad</option>
            ...
            {adddiv && (
              <FormDiv>
                <TitleBlock>
                  <HouseOutlined
                    style={{ paddingRight: '50px', color: 'white' }}
                  />{' '}
                  Shipping Address
                </TitleBlock>
                <div>
                  ...
                  <CloseButton onClick={() => setAddDiv(false)}>
                    {' '}
                    Close
                  </CloseButton>
                </div>
              ...
              {bol == true ? (
                <>
                  ...
                  <InputDiv>
                    ...
                    <Input
                      ...
                      placeholder='Street Address: Line 1'
                      value={accountship.user_address}
                  ...
                  <InputDivC>
                    <InputDivS>
                      <Select
                        ...
                        onChange={handleAChange}
                      >
                        <option value={accountship.country} disabled selected>
                          {accountship.country}
                        </option>
                        <option value='AF'>Afghanistan</option>
                        <option value='AX'>Åland Islands</option>
                    ...
                    <InputDivS>
                      <Select
                        ...
                        onChange={handleAChange}
                      >
                        <option value={accountship.province} disabled selected>
                          {accountship.province}
                        </option>
                        <option value='Azad Kashmir'>Azad Kashmir</option>
                        <option value='Balochistan'>Balochistan</option>
                        <option value='Islamabad'>Islamabad </option>
                ...
                <>
                  {title == false ? (
                    <>
                      <TitleBlock>
                        <HouseOutlined style={{ paddingRight: '50px' }} />{' '}
                        Shipping Address
                      </TitleBlock>
                      {shipp.map((item, i) => {
                        return (
                ...
                <FormDiv>
                  <label style={{ margin: '20px' }}>
                    <input
                      ...
                      value='billing'
                      checked={billing}
                      onChange={handleChangebillingList}
                    />
                    My billing and shipping address are the same
                  </label>
                </FormDiv>
              ...
              <FormDiv>
                ...
                <InputDiv>
                  ...
                  <Input
                    name='user_address'
                    placeholder='Street Address: Line 1'
                    value={accountbill.user_address}
                    onChange={handleEditAChange('user_address')}
                  />
                  <Error>{accountbill.user_address}</Error>
                  <Input
                    placeholder='Line 2'
                    {...register('Bstreet_address2')}
                  />
                </InputDiv>
                ...
                <InputDivC>
                  <InputDivS>
                    ...
                    <Select
                      name='country'
                      onChange={handleEditAChange('country')}
                    >
                      <option value='' disabled selected>
                        Country
                      </option>
                  ...
                  <InputDivS>
                    ...
                    <Select
                      name='province'
                      onChange={handleEditAChange('province')}
                    >
                      <option value='' disabled selected>
                        State/Province
                      </option>
                ...
                <InputDivC>
                  ...
                  <InputDivS>
                    ...
                    <Input
                      placeholder='Zip/ Postal code'
                      name='postal_code'
                      onChange={handleEditAChange('postal_code')}
                    />
            ...
            {editbilling && (
              <FormDiv>
                ...
                <InputDiv>
                  ...
                  />
                  <Error>
                    {errors.user_address1?.type === 'required' &&
                      'Street Address is required'}
                  </Error>
                  <Input
                    placeholder='Line 2'
                ...
                <InputDivC>
                  <InputDivS>
                    ...
                    >
                      <option value={address.country} disabled selected>
                        {address.country}{' '}
                      </option>
                      <option value='AF'>Afghanistan</option>
                      <option value='AX'>Åland Islands</option>
                  ...
                  <InputDivS>
                    ...
                    >
                      <option value={address.province} disabled selected>
                        {address.province}{' '}
                      </option>
                      <option value='Azad Kashmir'>Azad Kashmir</option>
                      <option value='Balochistan'>Balochistan</option>
                ...
                <InputDivC>
                  <InputDivS>
                    <Select
                      ...
                      {...register('city', { required: true })}
                    >
                      <option value={address.city} disabled selected>
                        {address.city}{' '}
                      </option>
                      <option value='Islamabad'>Islamabad</option>
                      <option value='' disabled>
                    ...
                    <Error>
                      {errors.Bcity?.type === 'required' && 'City is required'}
                    </Error>
                  </InputDivS>
          ...
          <Right>
            <FormDiv>
              <TitleBlock>
                <Payment style={{ paddingRight: '50px' }} /> Payment methods
              </TitleBlock>
                ...
                <div>
                  ...
                  <Label style={{ width: '600px' }}>
                    ...
                    <img
                      ...
                      src='https://freepngimg.com/thumb/mastercard/70535-credit-vector-card-mastercard-logo-free-frame.png'
                    />
                  </Label>
                  <Error>
                ...
                {account == true ? (
                  <>
                    ...
                    <InputDivC>
                      ...
                      {discount == true ? (
                        <img
                          src='https://www.kindpng.com/picc/m/721-7212637_done-icon-white-png-transparent-png.png'
            ...
            <FormDiv>
              <TitleBlock>
                <ShoppingBasket style={{ paddingRight: '50px' }} /> Order
                Summary
              </TitleBlock>
              {cartItems.map(item => (
                <>
              ...
              <Summary>
                ...
                <SummaryItem type='total'>
                  ...
                  {ptotal === null ? (
                    ...
                    <SummaryItemPrice>
                      <strong>{ptotal.toFixed(2)} Rs</strong>
                    </SummaryItemPrice>
                  )}
                  {/* </> */}
                </SummaryItem>
